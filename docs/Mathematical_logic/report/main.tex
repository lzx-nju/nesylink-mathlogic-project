...
\begin{itemize}[leftmargin=2em]
  ...
  \item Lean 当前验证的是符号层环境、显式 witness 计划，以及
  \texttt{task\_5} 高层阶段机策略的合法性与完成性；
...
为覆盖评分项中的“策略形式化与证明”，\texttt{Task5EnvironmentFormalization.lean}
在环境模型之后进一步定义了 \texttt{PolicyStage} 与 \texttt{symbolicPolicy}。
\texttt{PolicyStage} 把任务拆成
\texttt{openStartChest}、\texttt{pressStartButton}、\texttt{openSouthChest}、
\texttt{openEastChest}、\texttt{openWestChest} 和 \texttt{done} 六个阶段；
\texttt{symbolicPolicy} 根据当前房间、按钮状态、钥匙数和四个宝箱状态输出下一步房间级动作。

为了证明策略不是单纯列出路径，Lean 文件还定义了：
\begin{itemize}[leftmargin=2em]
  \item \texttt{policyTrace}：从当前状态反复调用 \texttt{symbolicPolicy} 生成动作轨迹；
  \item \texttt{runPolicy}：直接执行策略得到后继状态；
  \item \texttt{ActionEnabled}：刻画每个动作在当前状态下是否合法，例如
  \texttt{goSouth} 需要按钮已按，\texttt{goEast} 需要钥匙或处于西房返回分支；
...
\[
\begin{aligned}
&\texttt{policyTrace\_10\_matches\_witnessPlan}:\\
&\quad \texttt{policyTrace 10 initialState = witnessPlan};\\
&\texttt{policyTrace\_10\_actions\_enabled}:\\
&\quad \texttt{actionsEnabledAlong initialState (policyTrace 10 initialState)};\\
&\texttt{symbolicPolicy\_run\_10\_finalState}:\\
&\quad \texttt{runPolicy 10 initialState = finalState};\\
&\texttt{task5\_symbolicPolicy\_completes}:\\
&\quad \exists n,\ \texttt{GoalReached (runPolicy n initialState)}.
\end{aligned}
\]
这说明 \texttt{task\_5} 的 Lean 部分不再只证明“存在一条完成路径”，
...
\path{Task1EnvironmentFormalization.lean} & 通过 & Task 1 环境形式化 \\
\path{Task2EnvironmentFormalization.lean} & 通过 & Task 2 环境形式化 \\
\path{Task3EnvironmentFormalization.lean} & 通过 & Task 3 环境形式化 \\
\path{Task4EnvironmentFormalization.lean} & 通过 & Task 4 环境形式化 \\
\path{Task5EnvironmentFormalization.lean} & 通过 & Task 5 环境形式化与高层策略证明 \\
\bottomrule
\end{tabular}
