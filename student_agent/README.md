...
- 当前已实现 `task_1`、`task_2`、`task_3` 和 `task_4` 的可运行 baseline。
- 其中 `task_2` 当前采用从参考成功执行轨迹中提取的脚本化 debug 版本。
- `task_3` 当前采用像素房间识别 + 玩家中心格定位 + 物品栏钥匙数的规则状态机。
- `task_4` 当前采用像素房间识别、桥状态像素识别、玩家中心格定位和阶段记忆，依次完成拿钥匙、拿剑、击杀守卫和开最终宝箱。
...
3. 继续加强 `task_3` / `task_4` 的像素识别鲁棒性，并补充截图和失败样例记录。
4. 将 `task_4` 的阶段机和 Lean 形式化命题进一步对齐。
